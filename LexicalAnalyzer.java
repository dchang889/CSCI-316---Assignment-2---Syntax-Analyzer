...
public class LexicalAnalyzer {
	...
	public LexicalAnalyzer(String filename){
		...
		if(inputLine.size()==0) {
			System.out.println("Your file "+filename+" is empty.");
			System.exit(1);
		}
	...
	public Token getToken() {
		String lexeme = "";
		char currChar;
		currCharPosNum = cIndex;
		
		if (cIndex >= inputLine.get(currInputLineIndex).length()){
			resetCIndex();
			currInputLineIndex++;
			currCharPosNum = cIndex;
		}
		...
		while(currChar == ' ') {
			incrementCIndex(1);
			currCharPosNum = cIndex;
			if (inputLine.get(currInputLineIndex).length()==cIndex) {
				return getToken();
			}
			currChar = getCharacter(inputLine.get(currInputLineIndex));
		}
		...
		if(Character.isLetter(currChar)) {
			do {
				lexeme += currChar;	
				incrementCIndex(1);
				if (inputLine.get(currInputLineIndex).length()==cIndex) break;
				...
				if ((lexeme.toLowerCase()).equals("end") && currChar == ' ') {
					...
					String s2 = s1.substring(cIndex-3, Math.min(s1.length(),cIndex+9));
			...
			lexeme += currChar;																	
			incrementCIndex(1);
			if (inputLine.get(currInputLineIndex).length()==cIndex) return symbol(lexeme);					
			else currChar = getCharacter(inputLine.get(currInputLineIndex));
	...
	private void resetCIndex() {
		cIndex = 0;
		printNextLine();
	}

	private void incrementCIndex(int i) {
		for (int n = 0; n<i ; n++) {
			cIndex ++;
		}
	}
	...
	private void printNextLine() {
		System.out.println(inputLine.get(currInputLineIndex+1));
	}
